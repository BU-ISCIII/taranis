...
class ClusterDistance:
    ...
    def verify_cluster_quality(self , src_cluster_ptrs):
        ...
        for cluster_id in range(np.max(src_cluster_ptrs) + 1):
                ...
                avg_clusters[cluster_id] = self.calculate_mean_cluster(src_cluster_bool_ptrs)
    ...
    def create_clusters(self):
        comm_graph = ig.Graph.Weighted_Adjacency(self.dist_matrix.tolist(), mode=1, loops=False)
        graph_clusters = leidenalg.find_partition(comm_graph, leidenalg.CPMVertexPartition, weights='weight', n_iterations=-1, resolution_parameter=self.res_param, seed=self.seed)
        ...
        if not all(x > 0.9 for x in list(avg_clusters.values())):
            log.warning(f"There are some cluster below average of 0.9 in locus {self.ref_seq_name} ")
            stderr.print(f"[red]There are some cluster below average of 0.9 in locus {self.ref_seq_name}")
        return cluster_ptrs
